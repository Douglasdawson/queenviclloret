  ...
  return (
    <Section className="py-16 sm:py-24">
      <Container className="max-w-3xl">
        <h1 className="font-display display-2 font-bold text-ink-900">{doc.title}</h1>
        <p className="label-caps mt-3 text-xs text-ink-600">Updated {doc.updated}</p>
        <p className="mt-6 max-w-2xl text-[1.0625rem] leading-relaxed text-ink-600">{doc.intro}</p>
        <div className="mt-10 space-y-7">
          {doc.sections.map((s) => (
            <div key={s.h}>
              <h2 className="font-display text-lg font-bold text-green-900">{s.h}</h2>
              <p className="mt-1.5 max-w-2xl text-[0.9375rem] leading-relaxed text-ink-600 [overflow-wrap:anywhere]">
                {s.p}
              </p>
            </div>
          ))}
